...
if __name__ == '__main__':
    ...
    f = open("ModelFeatureUI_epoch1200_neuron32.txt", 'w')
    sys.stdout = f
...
#         adam = optimizers.Adam(clipvalue=0.5)
        ...
        checkpointer = ModelCheckpoint(filepath='modelsModelFeatureUI/weights' + str(k) + '.hdf5', 
...
#         plt.savefig('lossModelFeatureUI' + str(k) + '.png')
        ...
        plt.axvline(np.argmin(history.history['val_loss']), color='r')
        lgd = plt.legend(bbox_to_anchor=(1.05, 0.65), loc=2, borderaxespad=0.)
        plt.savefig('figModelFeatureUI_neuron32_epoch1200_featureLen20_' + str(k) + '.png', bbox_extra_artists=[lgd], bbox_inches='tight')
        ...
        model = load_model('modelsModelFeatureUI/weights' + str(k) + '.hdf5', custom_objects = {'precisionM': precisionM})
