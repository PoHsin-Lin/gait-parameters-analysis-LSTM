...
# sensorNum denotes the number of sensors we use, can be 1, 3, or 5
sensorNum = 1
...
#             print("epoch no.%d" % epoch)
        ...
        plt.savefig('resultSpace/detectLossUI_'+str(k)+'.png', bbox_extra_artists=[lgd1, lgd2, lgd3], bbox_inches='tight')
        # ------------------------------------------------------------------------------------------
    ...
    print("\n ----- Weighted Average -----")
    hsMeanErrorSum = 0
    for i, meanError in enumerate(hsErrorCV):
        hsMeanErrorSum = hsMeanErrorSum + meanError * lengthList[i]
    hsPrecisionSum = 0
    for i, precision in enumerate(hsPrecisionCV):
        hsPrecisionSum = hsPrecisionSum + precision * lengthList[i]
    hsRmseSum = 0
    for i, rmse in enumerate(hsRmseCV):
        hsRmseSum = hsRmseSum + rmse * lengthList[i]
    
    toMeanErrorSum = 0
    for i, meanError in enumerate(toErrorCV):
        toMeanErrorSum = toMeanErrorSum + meanError * lengthList[i]
    toPrecisionSum = 0
    for i, precision in enumerate(toPrecisionCV):
        toPrecisionSum = toPrecisionSum + precision * lengthList[i]
    toRmseSum = 0
    for i, rmse in enumerate(toRmseCV):
        toRmseSum = toRmseSum + rmse * lengthList[i]

    print("\nCV HS mean error: %f" % (sum(hsMeanErrorSum) / float(sum(lengthList))))
    print("CV HS precision: %f" % (sum(hsPrecisionSum) / float(sum(lengthList))))
    print("CV HS rmse: %f" % (sum(hsRmseSum) / float(sum(lengthList))))
    
    print("\nCV TO mean error: %f" % (sum(toMeanErrorSum) / float(sum(lengthList))))
    print("CV TO precision: %f" % (sum(toPrecisionSum) / float(sum(lengthList))))
    print("CV TO rmse: %f" % (sum(toRmseSum) / float(sum(lengthList))))
