...
def seperateData(X_seq, X_weight, Y_seq, k):
    ...
    tingTrain, tingTest, tingVali = seperateTrainValiTest(X_seq[sum(lengthList[0:3])*3:sum(lengthList[0:4])*3], 
                                                          X_weight[sum(lengthList[0:3])*3:sum(lengthList[0:4])*3], 
                                                          Y_seq[sum(lengthList[0:3])*3:sum(lengthList[0:4])*3], k, 3)
    thomasTrain, thomasTest, thomasVali = seperateTrainValiTest(X_seq[sum(lengthList[0:4])*3:sum(lengthList[0:5])*3], 
                                                          X_weight[sum(lengthList[0:4])*3:sum(lengthList[0:5])*3], 
                                                          Y_seq[sum(lengthList[0:4])*3:sum(lengthList[0:5])*3], k, 4)
    geneTrain, geneTest, geneVali = seperateTrainValiTest(X_seq[sum(lengthList[0:5])*3:sum(lengthList[0:6])*3], 
                                                          X_weight[sum(lengthList[0:5])*3:sum(lengthList[0:6])*3], 
                                                          Y_seq[sum(lengthList[0:5])*3:sum(lengthList[0:6])*3], k, 5)
...
#     print("slowVali[0].length = %d" % len(slowVali[0]))
    ...
    trainData = [np.concatenate((pohsinTrain[0], fastTrain[0], slowTrain[0], tingTrain[0], thomasTrain[0], geneTrain[0])),
                 np.concatenate((pohsinTrain[1], fastTrain[1], slowTrain[1], tingTrain[1], thomasTrain[1], geneTrain[1])), 
                 np.concatenate((pohsinTrain[2], fastTrain[2], slowTrain[2], tingTrain[2], thomasTrain[2], geneTrain[2]))]
    testData = [np.concatenate((pohsinTest[0], fastTest[0], slowTest[0], tingTest[0], thomasTest[0], geneTest[0])), 
                np.concatenate((pohsinTest[1], fastTest[1], slowTest[1], tingTest[1], thomasTest[1], geneTest[1]))]
    valiData = [np.concatenate((pohsinVali[0], fastVali[0], slowVali[0], tingVali[0], thomasVali[0], geneVali[0])), 
                np.concatenate((pohsinVali[1], fastVali[1], slowVali[1], tingVali[1], thomasVali[1], geneVali[1])), 
                np.concatenate((pohsinVali[2], fastVali[2], slowVali[2], tingVali[2], thomasVali[2], geneVali[2]))]
    testIndex = [len(pohsinTest[0]), len(fastTest[0]), len(slowTest[0]), len(tingTest[0]), len(thomasTest[0]), len(geneTest[0])]
...
if __name__ == '__main__':
    ...
    f = open("ModelFeatureUserDependent_epoch1500.txt", 'w')
    sys.stdout = f
...
#         adam = optimizers.Adam(clipvalue=0.5)
        ...
        checkpointer = ModelCheckpoint(filepath='modelsModelFeatureSpeed/weights' + str(k) + '.hdf5', 
...
#         plt.savefig('lossModelFeatureUI' + str(k) + '.png')
        ...
        plt.axvline(np.argmin(history.history['val_loss']), color='r')
        lgd = plt.legend(bbox_to_anchor=(1.05, 0.65), loc=2, borderaxespad=0.)
        plt.savefig('figModelFeatureUserDependent_epoch1500_' + str(k) + '.png', bbox_extra_artists=[lgd], bbox_inches='tight')
        ...
        model = load_model('modelsModelFeatureSpeed/weights' + str(k) + '.hdf5', custom_objects = {'precisionM': precisionM})
...
#         print("k = %d" % k)
        ...
        for i in range(len(testY)):
            ...
            elif i >= sum(testIndex[0:2]) and i < sum(testIndex[0:3]):
                lossListSlow.append(errorDis)
            elif i >= sum(testIndex[0:3]) and i < sum(testIndex[0:4]):
                lossListTing.append(errorDis)
            elif i >= sum(testIndex[0:4]) and i < sum(testIndex[0:5]):
                lossListThomas.append(errorDis)
            elif i >= sum(testIndex[0:5]) and i < sum(testIndex[0:6]):
                lossListGene.append(errorDis)
        ...
        meanErrorTing, precisionTing, rmseTing = meanErrorPrecisionRMSE(lossListTing)
        meanErrorThomas, precisionThomas, rmseThomas = meanErrorPrecisionRMSE(lossListThomas)
        meanErrorGene, precisionGene, rmseGene = meanErrorPrecisionRMSE(lossListGene)
        ...
        meanerrorListTing.append(meanErrorTing)
        precisionListTing.append(precisionTing)
        rmseListTing.append(rmseTing)
        meanerrorListThomas.append(meanErrorThomas)
        precisionListThomas.append(precisionThomas)
        rmseListThomas.append(rmseThomas)
        meanerrorListGene.append(meanErrorGene)
        precisionListGene.append(precisionGene)
        rmseListGene.append(rmseGene)
    ...
    print("\nAverage MeanError Ting: %f" % (sum(meanerrorListTing) / float(len(meanerrorListTing))))
    print("Average Precision Ting: %f" % (sum(precisionListTing) / float(len(precisionListTing))))
    print("Average Rmse Ting: %f" % (sum(rmseListTing) / float(len(rmseListTing))))
    print("\nAverage MeanError Thomas: %f" % (sum(meanerrorListThomas) / float(len(meanerrorListThomas))))
    print("Average Precision Thomas: %f" % (sum(precisionListThomas) / float(len(precisionListThomas))))
    print("Average Rmse Thomas: %f" % (sum(rmseListThomas) / float(len(rmseListThomas))))
    print("\nAverage MeanError Gene: %f" % (sum(meanerrorListGene) / float(len(meanerrorListGene))))
    print("Average Precision Gene: %f" % (sum(precisionListGene) / float(len(precisionListGene))))
    print("Average Rmse Gene: %f" % (sum(rmseListGene) / float(len(rmseListGene))))
